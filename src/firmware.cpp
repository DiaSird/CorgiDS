...
int Firmware::load_firmware(string file_name) {
  ...
  if (!firmware_file.is_open()) {
    ...
    auto fw = dust::default_firmware(dust::Model::Ds);
  ...
  if (*(uint16_t *)&firmware[user_data + 0x170] ==
      ((*(uint16_t *)&firmware[user_data + 0x70] + 1) & 0x7F)) {
  ...
  *(uint16_t *)&firmware[user_data + 0x72] =
      create_CRC(&firmware[user_data], 0x70, 0xFFFF);

  *(uint16_t *)&firmware[0x2A] =
      create_CRC(&firmware[0x2C], *(uint16_t *)&firmware[0x2C], 0x0000);

  printf("\nFW: USER0 CRC16 = %s",
         verify_CRC(0xFFFF, 0x3FE00, 0x70, 0x3FE72) ? "GOOD" : "BAD");
  printf("\nFW: USER1 CRC16 = %s",
         verify_CRC(0xFFFF, 0x3FF00, 0x70, 0x3FF72) ? "GOOD" : "BAD");
...
uint16_t Firmware::create_CRC(uint8_t *data, int length, uint32_t start) {
  uint16_t stuff[8] = {0xC0C1, 0xC181, 0xC301, 0xC601,
...
bool Firmware::verify_CRC(uint32_t start, uint32_t offset, uint32_t length,
                          uint32_t CRC_offset) {
