...
Emulator::Emulator()
    : arm7(this, 1), arm9(this, 0), arm9_cp15(this), cart(this), dma(this),
...
void Emulator::load_firmware(uint8_t *firmware) { spi.init(firmware); }

int Emulator::load_firmware() {
  ifstream arm9_bios_file(Config::arm9_bios_path, ios::in | ios::binary);

  if (!arm9_bios_file.is_open()) {
    memcpy(arm9_bios, melonDS::bios_arm9_bin.data(), 4096 * sizeof(uint8_t));
    // printf("Failed to load ARM9 BIOS\n");
    printf("Loaded ARM9 Free BIOS\n");
    // return 1;
  } else {
  ...
  if (!arm7_bios_file.is_open()) {
    memcpy(arm7_bios, melonDS::bios_arm7_bin.data(), 16384 * sizeof(uint8_t));
    // printf("Failed to load ARM7 BIOS\n");
    printf("Loaded ARM7 Free BIOS\n");
    // return 1;
  } else {
...
void Emulator::direct_boot() {
  ...
  printf("\nA9 offset: $%08X entry: $%08X RAM: $%08X size: $%08X", boot_info[0],
         boot_info[1], boot_info[2], boot_info[3]);
...
uint64_t Emulator::get_timestamp() { return system_timestamp; }

void Emulator::HBLANK_DMA_request() { dma.HBLANK_request(); }

void Emulator::gamecart_DMA_request() { dma.gamecart_request(); }

void Emulator::GXFIFO_DMA_request() { dma.GXFIFO_request(); }

void Emulator::check_GXFIFO_DMA() { gpu.check_GXFIFO_DMA(); }
...
void Emulator::cart_write_header(uint32_t address, uint16_t halfword) {
  *(uint16_t *)&main_RAM[(0x027FFE00 + (address & 0x1FF)) & MAIN_RAM_MASK] =
      halfword;
}
...
ARM_CPU *Emulator::get_arm9() { return &arm9; }

ARM_CPU *Emulator::get_arm7() { return &arm7; }

bool Emulator::arm7_has_cart_rights() { return EXMEMCNT & (1 << 11); }
...
void Emulator::start_division() {
  ...
  // printf("\nDivision: %lld / %lld = %lld (%lld)", DIV_NUMER, DIV_DENOM,
  // DIV_RESULT, DIV_REMRESULT);
}
